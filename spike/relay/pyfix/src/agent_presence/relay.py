...
class Relay:
    ...
    def broadcast(
        ...
        """SPIKE (algorithmic-fix prototype): encode once, fan out the same
        bytes to every member instead of letting each connection's own writer
        re-run json.dumps + permessage-deflate on an identical dict. This is
        the change the profiler pointed at -- see docs/relay-spike.md.
        `send_encoded` is a serve.WsConn method; a Conn without it (e.g. a
        test double) just gets the plain dict, unchanged."""
        targets = [c for c in self._members.get(room, []) if c is not exclude]
        text = json.dumps(apply_opaque(payload)) if opaque_enabled() else json.dumps(payload)
        for c in targets:
            fn = getattr(c, "send_encoded", None)
            if fn is not None:
                fn(text)
            else:
                c.send(payload)
