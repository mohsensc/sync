...
class WsConn:
    ...
    def send_encoded(self, text: str) -> None:
        ...
        SPIKE (algorithmic-fix prototype): `broadcast`/`publish` call this
        once per fan-out event with one shared string instead of handing
        every recipient the same dict and making each connection's `_drain`
        run `json.dumps` + permessage-deflate on it separately. Profiling the
        stock relay under tests/load's swarm shape showed exactly that
        per-recipient re-encode/re-compress as the dominant real cost, well
        ahead of the lease-table diff itself -- see docs/relay-spike.md.
        """
        self._enqueue(text)
...
async def serve(
    ...
    *,
    ...
    async with websockets.serve(
        ...
        # SPIKE (algorithmic-fix prototype): permessage-deflate is on by
        # default and profiling showed zlib.Compress.flush/compress as the
        # single largest non-idle self-time consumer in the relay under the
        # swarm shape -- ahead of json encoding, ahead of the lease diff.
        # These frames are a few hundred bytes of JSON; compressing them per
        # connection, per send, buys back little wire size and costs real
        # CPU. See docs/relay-spike.md.
        compression=None,
    ) as server:
