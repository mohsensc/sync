...
async def serve(
    ...
    *,
    ...
    async with websockets.serve(
        ...
        # (cpp/daemon, agent-presence-mcp, go/internal/relay all dial
        # 127.0.0.1) — bandwidth is free and latency is a memcpy. TLS
        # (docs/tls-dev-cert.md) covers a network-reachable hop's
        # confidentiality, not this: re-cost compression against a real
        # link before assuming "off" still wins there.
        compression=None,
        ssl=ssl_context,
    ) as server:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help="PEM certificate (env AGENT_PRESENCE_TLS_CERT). Terminates "
             ...
             "Python-relay only — gorelay doesn't terminate TLS yet.",
    )
    parser.add_argument(
        "--tls-key",
    ...
    if args.impl == "go":
        if args.tls_cert or args.tls_key:
            # gorelay has no TLS support of its own yet (docs/relay-parity.md).
            # Execing it anyway would silently drop --tls-cert/--tls-key on
            # the floor and serve plaintext ws:// where the operator asked
            # for wss:// — loud and fatal beats quiet and unencrypted.
            raise SystemExit(
                "--tls-cert/--tls-key are not supported with --impl go yet "
                "(gorelay doesn't terminate TLS). Drop them or use --impl python."
            )
