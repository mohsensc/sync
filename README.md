...
above plus an AGENTS.md/CLAUDE.md section from the hosted setup picker, which
the connecting agent has to pick up and follow on its own. Claude Code alone
gets the real hook + MCP wiring (see `api/_lib/tokens.ts`'s
`setupInstructions`). Packaging and self-registration: `docs/install-plan.md`.
