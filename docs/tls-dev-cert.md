...
Or `AGENT_SYNC_TLS_CERT` / `AGENT_SYNC_TLS_KEY`. `--host` still
...
```
AGENT_SYNC_RELAY=wss://<relay-host>:8799 \
AGENT_SYNC_RELAY_CA=relay-cert.pem \
./presenced
```

`AGENT_SYNC_RELAY_CA` trusts that one cert on top of the system root
...
Skip verification entirely with `AGENT_SYNC_RELAY_INSECURE_SKIP_VERIFY=1`
...
`agent-sync-mcp` (Go) dials `wss://` through the system root pool via
`websocket.DefaultDialer`, the same as any other Go TLS client — no
`AGENT_SYNC_RELAY_CA`-style override of its own yet. For a self-signed
