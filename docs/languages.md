...
| `similarity.py` (rung 4) | Conditional verdict, not settled. `LexicalSimilarity.score` runs ~166us; `redundant_peer` retokenizes both intent strings inside its own peer loop (confirmed by reading `ladder.py`'s `redundant_peer` — `scorer.score(intent, o.intent)` sits inside `for o in others`, so the incoming side's tokens get recomputed every iteration, not just the peer's), which scales with room size: ~0.8ms at 5 peers, 15.5ms at 200. `AGENT_PRESENCE_RUNG4` defaults off (confirmed: `ladder.py`'s `rung4_enabled()` checks an unset env var), so this costs nothing today, which is also why the Go relay skipped porting it. **If rung 4 ships enabled, this is a real Go candidate under storm; if it stays off, there is nothing to move.** Settling measurement: rerun the swarm scenario with `AGENT_PRESENCE_RUNG4=1` at 200 agents and read `relay_cpu_ms_per_op` — not yet done on either relay. |  `python/src/agent_presence/ladder.py`, `similarity.py` |
