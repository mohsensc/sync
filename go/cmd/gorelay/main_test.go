...
// AGENT_PRESENCE_PORT killed the process before the explicit --port that
// was there to override it was ever read.
func TestEnvPortReportsInsteadOfExiting(t *testing.T) {
	t.Setenv("AGENT_PRESENCE_PORT", "abc")
...
func TestEnvPortReadsAGoodValue(t *testing.T) {
	t.Setenv("AGENT_PRESENCE_PORT", "9123")
	n, set, err := envPort("AGENT_PRESENCE_PORT")
