...
// initialMigrationSQL is the binary-embedded counterpart of
// db/migrations/000001_hosted_accounts.up.sql. It is kept here because Go's
// embed directive cannot traverse above the go module root, while gorelay must
// be able to migrate in a deployment that contains only the compiled binary.
...
CREATE TABLE IF NOT EXISTS resolution_events (
    ...
    FOREIGN KEY (workspace_id, repository_id) REFERENCES repositories(workspace_id, id) ON DELETE CASCADE,
    FOREIGN KEY (workspace_id, actor_user_id) REFERENCES workspace_memberships(workspace_id, user_id) ON DELETE CASCADE,
    FOREIGN KEY (workspace_id, peer_user_id) REFERENCES workspace_memberships(workspace_id, user_id) ON DELETE SET NULL (peer_user_id),
    CONSTRAINT resolution_events_type_valid CHECK (event_type IN ('contention', 'handover', 'override', 'redundant')),
    CONSTRAINT resolution_events_outcome_valid CHECK (outcome IN ('wait', 'abort', 'handover', 'proceed', 'redundant')),
