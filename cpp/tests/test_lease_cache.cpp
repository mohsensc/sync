...
/// Fills in place rather than returning a cache: LeaseCache owns a mutex now,
/// so it is neither copyable nor movable, and it is only ever held by reference
/// anyway.
void with_one(LeaseCache& c, long long expires) {
